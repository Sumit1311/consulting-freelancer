...
int openOutput()
{
  outputFd = open(FIFO_OP_FILE_NAME, O_WRONLY);

  if (outputFd == -1)
  {
    printf("Can't open fifo pipe %s\n", strerror(errno));
    exit(1);
  }
  return outputFd;
}
...
int main(int argc, char *argv[])
{
  ...
  int rc = mkfifo(FIFO_IP_FILE_NAME, 0666);
  ...
  rc = mkfifo(FIFO_OP_FILE_NAME, 0666);
  ...
  inputFd = open(FIFO_IP_FILE_NAME, O_RDWR);
  ...
  inputFifoPipe = fdopen(inputFd, "a+");
  ...
  if (pid > 0)
  {
    ...
    printf("Starting the sm daemon with pid : %d...\n", (int)pid);
    exit(0);
  }
  ...
  sm_init();
  //int fd = open("./daemon_out.log", O_RDWR | O_APPEND | O_CREAT | O_TRUNC,S_IRWXU | S_IRWXO | S_IRWXG);

  //dup2(fd, fileno(stdout));
  //dup2(fileno(stdout), fileno(stderr));
  //dup2(outputFd, fileno(stdout));
  process_commands(inputFifoPipe);
  unlink(FIFO_IP_FILE_NAME);
  unlink(FIFO_OP_FILE_NAME);
  sm_free();
  return 0;
}
