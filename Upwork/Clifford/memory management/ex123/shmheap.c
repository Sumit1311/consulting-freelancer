...
shmheap_memory_handle shmheap_create(const char *name, size_t len)
{
    ...
    handle.size = len;
    memcpy(handle.name, name, 20) ;
    handle.next = NULL;

    initial_book.max_size = len;
    initial_book.next_shm_name[0] = NULL;
    ...
    heap_mutex = sem_open(SEM_NAME, O_RDWR | O_CREAT, S_IRWXG | S_IRWXO | S_IRWXU, 1);
...
shmheap_memory_handle shmheap_connect(const char *name)
{
    ...
    heap_mutex = sem_open(SEM_NAME, O_RDWR);
...
void shmheap_disconnect(shmheap_memory_handle mem)
{
    ...
    printf("Disconnect\n");
    sem_wait(heap_mutex);
    int rc = munmap(mem.heap_start, mem.size);
    if (rc == -1)
    {
...
{
    /* TODO */
    printf("shm_heap_destory\n");
    int rc = sem_unlink(SEM_NAME);
    if (rc == -1)
    {
        printf("Sem unlink failed with %s\n", strerror(errno));
    }
...
void *shmheap_alloc(shmheap_memory_handle mem, size_t sz)
{
    ...
    while (1)
    {
        ...
        {
            //sem_post(&heap_mutex);
            char file_name = mem.name;
            shmheap_memory_handle next = shmheap_create();
            continue;
            //sem_post(heap_mutex);
            //return NULL;
        }
...
void shmheap_free(shmheap_memory_handle mem, void *ptr)
{
    ...
    for (; (start_block + sizeof(shmheap_bk)) <= (void *)(mem.heap_start + mem.size); start_block += (bk->block_size + sizeof(shmheap_bk)))
    {
        ...
        if (bk->isOccupied == OCCUPIED_BLOCK || (bk->isOccupied == FREE_BLOCK && bk->block_size == 0))
        {
            break;
        }
        else
        {
            ...
            memset((start_block + sizeof(shmheap_bk)), 0, bk->block_size);
            bk->block_size = 0;
            bk->isOccupied = FREE_BLOCK;
        }
    }
    //sem_post(&heap_mutex);
    sem_post(heap_mutex);
