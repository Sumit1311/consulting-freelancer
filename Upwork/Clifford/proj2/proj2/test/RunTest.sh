...
run() {
  ...
  for p0 in $(genRandPorts); do
    ...
    (eval exec "$bcmd" $port > "$tmpdir/$cid.stdout" 2> "$tmpdir/$cid.bob.stderr") &
    bpid="$!"
    ...
    while [[ $tries -lt 3 ]]; do
      ...
      (eval exec "$acmd" $port2 < "$inf" 2> "$tmpdir/$cid.alice.stderr") &
      apid="$!"
  ...
  if [[ $added -ne 1 ]]; then
    ...
    res=${PIPESTATUS[0]}
    tput rev;
    cp "$tmpdir/$cid.stdout" ./$cid$ver.stdout
    cp "$tmpdir/$cid.alice.stderr" ./$cid$ver.alice.stderr
    cp "$tmpdir/$cid.bob.stderr" ./$cid$ver.bob.stderr
    cp "$inf" ./$cid$ver.in
    if [[ $res == "0" ]];
    then
      echo "Passed!"
